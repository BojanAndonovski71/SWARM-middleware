...
def Control_motors(m):
	...
	d0 = -np.int8(np.sign(m0))
	d1 = -np.int8(np.sign(m1))
	d2 = -np.int8(np.sign(m2))
	if d0 == -1 : d0 = 0
	if d1 == -1 : d1 = 0
	if d2 == -1 : d2 = 0
	s0 = np.uint8(abs(m0))
...
if __name__ == '__main__':
	...
	while not rospy.is_shutdown():
		...
		try:
			Check_sensors()
      time.sleep(0.05)
			m = rospy.wait_for_message("/motors", Vector3, timeout=1 ) # get the last latched topic 
      Control_motors (m)
		except:
			print('skip')
		time.sleep(0.2)
